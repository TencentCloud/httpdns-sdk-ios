...
- (id)initWithCapacity:(size_t)numBytes
{
	if ((self = [super init]))
	{
		...
		preBuffer = (unsigned char*)malloc(preBufferSize);
...
- (void)ensureCapacityForWrite:(size_t)numBytes
{
	...
	if (numBytes > availableSpace)
	{
		...
		uint8_t *newPreBuffer = (unsigned char*)realloc(preBuffer, newPreBufferSize);
...
- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes
{
	...
	uint8_t *buff = (unsigned char*)[buffer mutableBytes];
